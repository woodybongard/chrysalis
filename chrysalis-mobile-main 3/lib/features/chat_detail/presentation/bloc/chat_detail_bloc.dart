...
class ChatDetailBloc extends Bloc<ChatDetailEvent, ChatDetailState> {
  ChatDetailBloc(this.getMessagesUseCase, this.sendMessageUseCase)
    ...
    on<SingleMessageStatusUpdatedEvent>(_onSingleMessageStatusUpdated);
  ...
  /// Handles crypto decryption and socket room joining in background
  Future<void> _onInitializeChat(
    InitializeChatEvent event,
    Emitter<ChatDetailState> emit,
  ) async {
    log('=== InitializeChat START ===');
    log('Type: ${event.type}, ID: ${event.id}, isGroup: ${event.isGroup}');
    emit(const ChatDetailLoading());

    try {
      // 1. Load crypto keys
      log('Loading crypto keys...');
      final crypto = CryptoService();
      await crypto.loadKeys();
      log('Crypto keys loaded');

      // 2. Decrypt the group key
      log('Decrypting group key...');
      final decryptGroupKey = await crypto.decryptGroupSenderKey(
        event.encryptedGroupKey,
      );
      log('Group key decrypted, length: ${decryptGroupKey.length}');
      ...
      if (decryptGroupKey.length != 16 &&
        ...
        emit(const ChatDetailError('Invalid encryption key'));
        return;
      }

      // 3. Join socket room (fire and forget - don't await)
      log('Joining socket room...');
      _chatListHelper.joinConversation(
        conversationId: event.id,
        isGroup: event.isGroup,
      );

      // 4. Cache the group key and load messages
      _cachedGroupKey = decryptGroupKey;
      ...
      log('Messages loaded: ${data.messages.length}');

      final processedMessages = _attachGroupKeyToMessages(
        data.messages,
        _cachedGroupKey!,
      );

      final updatedList = setShowAvatarAndName(processedMessages);
      log('Emitting ChatDetailLoaded with ${updatedList.length} messages');
      emit(
        ChatDetailLoaded(
          type: event.type,
          id: event.id,
      ...
      log('=== InitializeChat SUCCESS ===');
    } catch (e, s) {
      log('=== InitializeChat ERROR ===');
      log('Error initializing chat: $e', stackTrace: s);
      // Fallback to empty loaded state
      emit(
        ChatDetailLoaded(
          type: event.type,
          id: event.id,
  ...
  void _onSingleMessageStatusUpdated(
    SingleMessageStatusUpdatedEvent event,
    Emitter<ChatDetailState> emit,
  ) {
    final current = state;
    if (current is! ChatDetailLoaded) return;
  ...
  /// Public getter for the cached group key (used for sending messages)
  encrypt.Key? get cachedGroupKey => _cachedGroupKey;

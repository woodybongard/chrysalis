...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
    ...
    if (currentState is HomeLoaded) {
      ...
      if (index != -1) {
        ...
        if (group.lastMessage != null &&
          ...
          // Only decrypt if it's not a FILE message
          final decryptedGroupKey = group.lastMessage!.type == 'FILE' 
              ? null 
              : await CryptoService().decryptGroupSenderKey(
  ...
  Future<void> _onNewMessageReceivedEvent(
    ...
    if (currentState is HomeLoaded) {
      ...
      // Only decrypt group key if the message is not a FILE type
      if (event.groupEntity.lastMessage?.type == 'FILE') {
        // For FILE messages, use the entity as is without decryption
        updated = event.groupEntity;
      } else {
        // For TEXT messages, decrypt the group key
        final crypto = CryptoService();
        final decryptedGroupKey = await crypto.decryptGroupSenderKey(
          event.groupEntity.lastMessage!.encryptedGroupKey,
        );
        updated = event.groupEntity.copyWith(
  ...
  Future<HomeEntity> _decryptAndUpdateMessages(HomeEntity messages) async {
    ...
    for (final group in messages.data) {
      ...
      } else if (group.lastMessage?.encryptedGroupKey != null) {
        final decryptedGroupKey = await crypto.decryptGroupSenderKey(

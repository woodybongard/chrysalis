...
class ChatListHelper {
  ...
  void listenForNewMessages(void Function(GroupEntity) onNewMessage) {
    ...
    _socketService.socket?.on('new_message', (data) {
      ...
      // Emit message:delivered for the last message if it's not from the current user
      if (groupEntity.lastMessage != null && !groupEntity.lastMessage!.isSenderYou) {
        emitMessageDelivered(messageId: groupEntity.lastMessage!.id);
      }
    });
  }
  ...
  void listenForChatMessages(void Function(MessageEntity) onNewMessage) {
    ...
    _socketService.socket?.on('group_message', (data) {
      ...
      // Only emit delivered for messages from other users
      if (!msgEntity.isSenderYou) {
        emitMessageDelivered(messageId: msgEntity.id);
      }
    });
  }
  ...
  void listenForConversationalMessages(
    ...
    _socketService.socket?.on('conv_message', (data) {
      ...
      if (!messageEntity.isSenderYou) {
        emitMessageDelivered(messageId: messageEntity.id);
      }
    });
  }
  ...
  Future<void> markRead({
    ...
    SocketLogger.log('mark_read', data);
    _socketService.socket?.emit('mark_read', data);
  }
  ...
  Future<void> markAllMessagesAsRead({
    ...
    for (final messageId in messageIds) {
      await markRead(chatId: chatId, messageId: messageId, isGroup: isGroup);
    }
  }
  ...
  Future<void> emitMessageDelivered({
    ...
    final userId = await _localStorage.read(key: AppKeys.userID);
    if (userId == null || userId.isEmpty) {
      SocketLogger.log('message:delivered', 'No userId found in local storage.');
      return;
    }
    final data = {
      'messageId': messageId,
    ...
    SocketLogger.log('message:delivered', data);
    _socketService.socket?.emit('message:delivered', data);
  }

  void listenForMessageAllDelivered(
    void Function({required String messageId, required String status}) onDelivered,
  ) {
    _socketService.socket?.off('message:all_delivered');
    _socketService.socket?.on('message:all_delivered', (data) {
      final mapData = data is String
          ? jsonDecode(data) as Map<String, dynamic>
          : data as Map<String, dynamic>;
      SocketLogger.log('message:all_delivered', mapData);
      final messageId = mapData['messageId'] as String? ?? '';
      final status = mapData['status'] as String? ?? 'DELIVERED';
      onDelivered(messageId: messageId, status: status);
    });
  }

...
class ChatDetailBloc extends Bloc<ChatDetailEvent, ChatDetailState> {
  ...
  Future<void> _onSendFileMessage(
    ...
    // 2. Create optimistic (temp) file message
    final tempMessage = MessageEntity(
      ...
      type: 'FILE',
      status: 'SENDING',
      fileUrl: savedFilePath, // local path for optimistic UI
  ...
  Future<List<MessageEntity>> _decryptAndUpdateMessages(
    List<MessageEntity> messages,
  ) async {
    final crypto = CryptoService();

    final updatedMessages = <MessageEntity>[];
    for (final message in messages) {
      final decryptedGroupKey = await crypto.decryptGroupSenderKey(
        message.encryptedGroupKey,
      );
      final updatedMessage = message.copyWith(decryptGroupKey: decryptedGroupKey);
      updatedMessages.add(updatedMessage);
    }
    return updatedMessages;
  }

  /// Handles retrying to send the same message instance (not a new copy)
  Future<void> _onLoadMessages(
    ...
    try {
      ...
      var processedMessages = data.messages;
      await _decryptAndUpdateMessages(data.messages).then((decryptedMessages) {
        processedMessages = decryptedMessages;
      });
  ...
  Future<void> _onPrependMessage(
    ...
    final crypto = CryptoService();
    final decryptedKey = await crypto.decryptGroupSenderKey(
      event.chatMessages.encryptedGroupKey,
    );

    // create a new object with the decryptedKey
    final updatedMessage = event.chatMessages.copyWith(
      decryptGroupKey: decryptedKey,
    );
  ...
  Future<void> _onSendMessage(
    ...
    // 1. Create optimistic (temp) message
    final tempMessage = MessageEntity(
      id: tempId,
      conversationId: current.id == 'conversation' ? current.id : null,
      groupId: current.type == 'group' ? current.id : null,
    ...
    try {
      ...
      final updatedMessages = latest.messages.map((m) {
        if (m.id == tempId) {
          return sent.copyWith(decryptGroupKey: decryptedGroupKey);
        }
        return m;
      }).toList();
  ...
  Future<void> _onRetrySendMessage(
    ...
    try {
      ...
      // 3. Replace retry message with sent
      final latest = state;
      if (latest is! ChatDetailLoaded) return;
      await _decryptAndUpdateMessages([sent]).then((decryptedMessages) {
        if (decryptedMessages.isNotEmpty) {
          sent = decryptedMessages.first;
        }
      });
      final updatedMessages = latest.messages.map((m) {
        if (m.id == retryMessage.id) return sent;
        return m;
      }).toList();
      final updatedList = setShowAvatarAndName(updatedMessages);
      emit(
